...
# ========== КОНФИГУРАЦИЯ ==========
TOKEN = '8032006876:AAE4b7z902XbYYQQ8VIW2J7kmIHTu8zVkO8'
# ==================================
...
async def show_my_movies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
...
async def show_watched(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    watched = db.get_movies(user.id, 'watched')
...
async def show_all_movies(update: Update, context: ContextTypes.DEFAULT_TYPE = None, query=None, user_id=None):
    if user_id is None and update:
        user_id = update.effective_user.id
    elif user_id is None and query:
...
async def show_public_movies(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def toggle_privacy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ ID фильма должен быть числом!")
        return
    
    movie = db.get_movie_by_id(user.id, movie_id)
    if not movie:
        await update.message.reply_text("❌ Фильм не найден или у вас нет доступа к нему!")
        return
    ...
    if new_state is not None:
        ...
        await update.message.reply_text("❌ Не удалось изменить приватность фильма.")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'show_lists':
        await show_lists_menu(query, user.id)
        ...
        if success:
            ...
            await query.edit_message_text("❌ Не удалось найти фильм. Возможно, он уже удален.")
        ...
        await show_all_movies(query=query, user_id=user.id)
...
async def show_lists_menu(query, user_id):
...
def main():
    ...
    try:
        ...
        application.add_handler(CommandHandler("my_movies", show_my_movies))
        application.add_handler(CommandHandler("watched", show_watched))
        application.add_handler(CommandHandler("all_movies", show_all_movies))
        application.add_handler(CommandHandler("public", show_public_movies))
        application.add_handler(CommandHandler("private", toggle_privacy))
        
        application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, add_movie))
...
if __name__ == '__main__':
    if TOKEN == '8032006876:AAE4b7z902XbYYQQ8VIW2J7kmIHTu8zVkO8':
        print("⚠️ ВНИМАНИЕ: Вы используете тестовый токен!")
        print("Замените переменную TOKEN на свой настоящий токен от @BotFather")
        print("\nКак получить токен:")
        print("1. Откройте Telegram")
        print("2. Найдите @BotFather")
        print("3. Отправьте /newbot")
        print("4. Следуйте инструкциям")
        print("5. Скопируйте токен и вставьте в код")
        print("\nПродолжить с тестовым токеном? (бот может не работать)")
        
        response = input("Введите 'y' для продолжения или любой другой символ для отмены: ")
        if response.lower() != 'y':
